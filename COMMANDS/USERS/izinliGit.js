...
if (!kullanıcı) return message.channel.send(furkyEmbed.setDescription(`Lütfen yanına gitmek istediğiniz **kullanıcıyı etiketleyin!** \n .izinligit @Furky/ID`)).then(m => m.delete(({ timeout: 5000 })));
if (!kullanıcı.voice.channel) return message.channel.send(furkyEmbed.setDescription(`Yanına gitmek istediğiniz kullanıcı **herhangi bir sesli kanalda bulunmuyor!**`)).then(m => m.delete(({ timeout: 5000 })));
...
.setColor("RANDOM")
.setDescription(`${message.author} kullanıcısı bulunduğunuz kanala gelmek istiyor, izin veriyor musunuz? \n**Bu işlem 60 saniye sonra iptal olacaktır!**`)
...
const reaction = collected.first();
  if (reaction.emoji.name === "✅") {
    ...
    mesaj.edit(furkyEmbed.setDescription(`${kullanıcı} kullanıcısı odaya giriş teklifinizi **kabul etti!**`)).then(m => m.delete(({ timeout: 10000 })));
  } else {
    await mesaj.edit(furkyEmbed.setDescription(`${kullanıcı} kullanıcısı odaya giriş teklifinizi **reddetti!**`)).then(m => m.delete(({ timeout: 10000 })));
  }
});
