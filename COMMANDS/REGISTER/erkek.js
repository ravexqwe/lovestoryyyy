...
const age = args[2];
const erkekRol1 = await data.fetch(`erkekRol1.${message.guild.id}`);
...
return message.react(config.GuildSettings.Emojis.crossEmoji);
  ...
  if(furky.roles.cache.has(erkekRol1)) return message.lineReply(furkyEmbed.setDescription(`Kullanıcı zaten sunucumuzda kayıtlı!`)).then(m => m.delete(({ timeout: 10000 })));
...
await furky.roles.add(erkekRol1);
...
await data.set(`kayıtTürü.${furky.id}`, `Erkek`);
