...
if (!kullanıcı) {
  return message.channel.send(furkyEmbed.setDescription(`Lütfen yanınıza çekmek istediğiniz bir **kullanıcı etiketleyin!**`)).then(m => m.delete(({ timeout: 5000 })));
};
...
await message.channel.send(furkyEmbed.setDescription(`${kullanıcı} kullanıcısını başarıyla **odanıza çektiniz!**`)).then(m => m.delete(({ timeout: 10000 })));
